...
func TestTimelineSnapshotBeforeFirstRequest(t *testing.T) {
	srv := newSrv(&config.Config{
		Routes: []config.Route{
			{
				Path:   "/jobs/1",
			...
			{Path: "/plain", Method: "GET", Status: 204},
		},
	})
	...
	if len(timelines) != 1 {
		t.Fatalf("expected only timeline routes in snapshot, got %+v", timelines)
	}
	got := timelines[0]
	if got.Key != "config-0" || got.Method != "GET" || got.Path != "/jobs/1" || got.Source != "config" {
		t.Fatalf("unexpected timeline identity: %+v", got)
	}
	if got.Step != 0 || got.Calls != 0 || got.Total != 2 || got.Complete {
		t.Fatalf("unexpected initial timeline progress: %+v", got)
	}
}

func TestTimelineResetMissingReturns404(t *testing.T) {
	srv := newSrv(&config.Config{
		Routes: []config.Route{
			{
				Path:     "/jobs/1",
				Method:   "GET",
				Timeline: []config.RouteResponse{{Status: 200, Response: map[string]any{"ok": true}}},
			},
		},
	})

	w := do(srv, "POST", "/__specter/timelines/missing/reset", `{}`)
	if w.Code != http.StatusNotFound {
		t.Fatalf("expected 404, got %d: %s", w.Code, w.Body.String())
	}
	body := jsonBody(t, w)
	if body["error"] != "timeline not found" {
		t.Fatalf("unexpected error body: %v", body)
	}
}

func TestResetAllClearsTimelineProgress(t *testing.T) {
	srv := newSrv(&config.Config{
		Routes: []config.Route{
			{
				Path:   "/jobs/1",
	...
	if len(timelines) != 1 || timelines[0].Step != 0 || timelines[0].Calls != 0 {
		t.Fatalf("expected reset-all to clear timeline progress, got %+v", timelines)
	}
}
...
func TestDynamicRouteTimelineLifecycle(t *testing.T) {
	srv := newSrv(&config.Config{})

	w := postRoute(srv, `{"path":"/dynamic-job","method":"GET","timeline":[{"status":202,"response":{"status":"queued"}},{"status":200,"response":{"status":"done"}}]}`)
	if w.Code != http.StatusCreated {
	...
	if created.ID == "" {
		t.Fatalf("expected dynamic route id")
	}
	time.Sleep(20 * time.Millisecond)

	w = do(srv, "GET", "/dynamic-job", "")
	if w.Code != http.StatusAccepted {
		t.Fatalf("expected first step 202, got %d: %s", w.Code, w.Body.String())
	}
	body := jsonBody(t, w)
	if body["status"] != "queued" {
		t.Fatalf("expected queued step, got %v", body)
	}
	w = do(srv, "GET", "/dynamic-job", "")
	if w.Code != http.StatusOK {
		t.Fatalf("expected second step 200, got %d: %s", w.Code, w.Body.String())
	}
	body = jsonBody(t, w)
	if body["status"] != "done" {
		t.Fatalf("expected done step, got %v", body)
	}
	...
	if len(timelines) != 1 || timelines[0].Key != created.ID || timelines[0].Source != "dynamic" || timelines[0].Step != 2 || timelines[0].Calls != 2 {
		t.Fatalf("unexpected dynamic timeline snapshot: %+v", timelines)
	}

	w = do(srv, "POST", "/__specter/timelines/"+created.ID+"/reset", `{}`)
	if w.Code != http.StatusOK {
		t.Fatalf("expected dynamic timeline reset 200, got %d: %s", w.Code, w.Body.String())
	}
	w = do(srv, "GET", "/dynamic-job", "")
	body = jsonBody(t, w)
	if body["status"] != "queued" {
		t.Fatalf("expected dynamic timeline to restart, got %v", body)
	}
}
...
func TestScenarioPreset_applyReplacesStateVarsAndStores(t *testing.T) {
	srv := newSrv(&config.Config{
		Scenarios: map[string]config.Scenario{
			"login-success": {
				State: "logged_in",
				Vars:  map[string]string{"role": "admin", "tier": "gold"},
	...
	srv.state.Set("old")
	srv.vars.Set("role", "user")
	srv.vars.Set("stale", "true")
	srv.store.SetCollection("users", []map[string]any{{"id": "old", "name": "Old"}})
	srv.store.SetCollection("orders", []map[string]any{{"id": "old"}})

	w := do(srv, "POST", "/__specter/scenarios/login-success", "")
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d: %s", w.Code, w.Body.String())
	}
	if srv.state.Get() != "logged_in" {
		t.Errorf("expected state logged_in, got %q", srv.state.Get())
	}
	gotVars := srv.vars.All()
	if gotVars["role"] != "admin" || gotVars["tier"] != "gold" || gotVars["stale"] != "" {
		t.Errorf("unexpected vars after scenario: %v", gotVars)
	}
	users := srv.store.List("users")
	if len(users) != 1 || users[0]["id"] != "1" || users[0]["name"] != "Alice" {
		t.Errorf("unexpected users store: %v", users)
	}
	if orders := srv.store.List("orders"); len(orders) != 0 {
		t.Errorf("expected stores to be replaced, got orders=%v", orders)
	}

	w = do(srv, "GET", "/__specter/scenario", "")
	var active map[string]string
	json.NewDecoder(w.Body).Decode(&active)
	if active["active"] != "login-success" {
		t.Errorf("expected active scenario login-success, got %v", active)
	}
}
...
func TestScenarioPreset_listAndNotFound(t *testing.T) {
	...
	w := do(srv, "GET", "/__specter/scenarios", "")
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", w.Code)
	}
	var payload struct {
	...
	if payload.Active != "" {
		t.Errorf("expected no active scenario, got %q", payload.Active)
	}
	if len(payload.Scenarios) != 2 || payload.Scenarios[0] != "a" || payload.Scenarios[1] != "b" {
		t.Errorf("expected sorted scenarios [a b], got %v", payload.Scenarios)
	}

	w = do(srv, "POST", "/__specter/scenarios/missing", "")
	if w.Code != http.StatusNotFound {
		t.Fatalf("expected 404, got %d", w.Code)
	}
}

func TestScenarioPreset_resetClearsActiveScenario(t *testing.T) {
	srv := newSrv(&config.Config{
		Scenarios: map[string]config.Scenario{
			"ready": {State: "ready"},
		},
	...
	w = do(srv, "POST", "/__specter/reset", `{"targets":["scenario"]}`)
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d", w.Code)
	}
	if srv.scenario.Get() != "" {
...
func TestStore_ResetDoesNotMutateConfigSeed(t *testing.T) {
	srv := newSrv(&config.Config{
		Stores: map[string]config.StoreConfig{
			"users": {
				Seed: []map[string]any{{"id": "seed", "name": "Seeded"}},
	...
	w := do(srv, "PATCH", "/users/seed", `{"name":"Changed"}`)
	if w.Code != http.StatusOK {
		t.Fatalf("patch seeded item: expected 200, got %d: %s", w.Code, w.Body.String())
	}
	w = do(srv, "POST", "/__specter/reset", `{"targets":["stores"]}`)
	if w.Code != http.StatusOK {
		t.Fatalf("reset: expected 200, got %d: %s", w.Code, w.Body.String())
	...
	if len(users) != 1 || users[0]["name"] != "Seeded" {
		t.Fatalf("expected reset to restore original seed value, got %v", users)
	}
}
...
func TestStoreFileEmptyFileStartsFromConfigSeed(t *testing.T) {
	path := t.TempDir() + "/stores.json"
	if err := os.WriteFile(path, nil, 0644); err != nil {
		t.Fatal(err)
	}
	...
	if len(users) != 1 || users[0]["id"] != "seed" {
		t.Fatalf("expected seed to initialize empty store file, got %v", users)
	}
	stores := readStoreFile(t, path)
	if len(stores["users"]) != 1 || stores["users"][0]["id"] != "seed" {
		t.Fatalf("expected seed to be written to empty store file, got %v", stores)
	}
}
...
func TestStoreFileResetRestoresConfigSeedAfterExistingFileWins(t *testing.T) {
	path := t.TempDir() + "/stores.json"
	if err := os.WriteFile(path, []byte(`{"users":[{"id":"file","name":"From File"}]}`), 0644); err != nil {
		t.Fatal(err)
	}
	...
	stores := readStoreFile(t, path)
	if len(stores["users"]) != 1 || stores["users"][0]["id"] != "seed" || stores["users"][0]["name"] != "Seeded" {
		t.Fatalf("expected reset to restore config seed after existing file wins, got %v", stores)
	}
}
